...
Predicting_migration <- function(DT_GT_wn){
  ...
  tsdata <- list(tsdata_total, tsdata_asy_OECD,tsdata_asy_UNHCR)
  return(list(tsdata,forecast_mig))
  
}
# A <- Predicting_migration(DT_GT_wn)
# 
# autoplot(A[[1]][[1]]) + autolayer(A[[2]][[1]])
...
GT_data <- GT_data_weekly(serach_terms = 'Germany', origin_code = 'SY',
         language = "en",
         category = 555)
...
B <- Predicting_GT(GT_data_weekly(serach_terms = 'Germany', origin_code = 'SY',
                                  language = "en",
                                  category = 555))

A <- Predicting_migration(Dashboard_DT_without_norm(Migrationdata, asylum_applications_UNHCR,
                                               origin = "Syria",destination = "Germany",
                                               Year_from = 2005,Year_till = 2018,
                                               origin_asylum = "Syria",
                                               destination_asylum = "Germany",
...
# Migration part
Scale_x <- scale_x_continuous(breaks = waiver(), n.breaks = 8)
# Migraiton total
mig_pre <- autoplot(A[[1]][[1]]) + autolayer(A[[2]][[1]])+ 
    labs(title ="Migration Inflows", y = "", x= "") + Scale_x +
    ylim(0,12e+5)
# Asylum OECD
OECD_asy_pre <- autoplot(A[[1]][[2]]) + autolayer(A[[2]][[2]])+
  labs(title ="Asylum OECD", y = "",x= "") + Scale_x+
  ylim(0,12e+5)
  # Asylum UNHCR
UNHCR_pre <- autoplot(A[[1]][[3]]) + autolayer(A[[2]][[3]]) +
    labs(title ="Asylum UNHCR", y = "",x= "") +
    ylim(0,12e+5)
